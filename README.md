<h1 align="center">Hey there 👋, I'm Vinit Sharma</h1>
<h3 align="center">
M.Tech (AI & Data Science) @ VJTI · AI/ML Researcher · LLM & Quantum ML Enthusiast
</h3>

---

## 🧠 About Me

I am a second-year **M.Tech student in Artificial Intelligence & Data Science at VJTI, Mumbai**, with a strong interest in **AI research, large language models, and intelligent systems**.

My work lies at the intersection of **multilingual NLP, ASR–LLM pipelines, retrieval-augmented generation (RAG), and emerging quantum-enhanced learning methods**. I am currently involved in applied research projects with real-world impact, including **AI-driven FIR automation systems** and **domain-specific LLM reasoning frameworks**.

I am actively preparing for **PhD programs in AI / ML**, with research interests spanning:
- Large Language Models & Reasoning
- Multilingual NLP and Speech-to-Text systems
- Retrieval-Augmented Generation (RAG)
- AI systems for law, governance, and public infrastructure
- Quantum Machine Learning

---

## 🔬 Current Work

- **M.Tech Thesis**: Multilingual, end-to-end FIR automation using ASR, LLMs, and RAG-based legal reasoning (in collaboration with Mumbai Police)
- **Research**: Quantum-based enhanced transformer architectures (manuscript under peer review)
- **Teaching Assistant**: Undergraduate Machine Learning laboratory courses at VJTI

---

## 🛠️ Technical Skills

**Programming & Frameworks**
- Python, PyTorch
- LangChain, RAG pipelines
- LoRA / QLoRA
- Qiskit, TorchQuantum

**Machine Learning & AI**
- Large Language Models (LLMs)
- Transformers, Attention Mechanisms
- Vision Transformers (ViT)
- Multilingual NLP, ASR pipelines

**Quantum & Mathematical Foundations**
- Quantum Computing
- Variational Quantum Circuits
- Linear Algebra, Probability, Statistics

**Data & Systems**
- AWS (EC2, S3, VPC, SageMaker, Glue, Athena)
- MySQL
- Docker
- Tableau & Data Visualization

---

## 🤝 Collaboration & Research

I enjoy working in **research-driven, collaborative environments** and contributing to:
- Open-source ML / NLP projects
- Applied AI systems
- Academic research and experimentation

I am open to **research collaborations, discussions, and mentorship opportunities** in AI and ML.

---

## 📫 Connect With Me

- 💼 [LinkedIn](https://www.linkedin.com/in/vinit-sharma-82460a217)
- 🧠 GitHub: [@sVinit108](https://github.com/sVinit108)
- 📧 Email: vdsharma_m24@ce.vjti.ac.in

---

## 📊 GitHub Stats

[![GitHub stats](https://github-readme-stats.vercel.app/api?username=sVinit108&show_icons=true&theme=merko)](https://github.com/sVinit108)


<!-- <h1 align="center">Hi 👋, Vinit Sharma here</h1>
<h3 align="center">A Passionate Python and ML Developer & AWS enthusiastic</h3>

## 🚀 About Me
I'm Vinit, a Second year Mtech (AI&DS) student @VJTI


Skills:  
- Python
- Artificial Intelligence
- Quantum Computing
- Data Analytics
- Tableau | Data Visualization

Also familiar with  
- AWS (Basic knowledge of AWS services viz. EC2,VPC,S3,Sagemaker, Glue, Athena)
- MySQL
- Docker




[![GitHub stats](https://github-readme-stats.vercel.app/api?username=sVinit108&theme=merko&show_icons=true)](https://github.com/sVinit108)


<!--
**sVinit108/sVinit108** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
 -->
